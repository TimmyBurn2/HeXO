...
export class SocketServerGateway {
    ...
    attach(server: HttpServer) {
        ...
        this.sessionManager.setEventHandlers({
            lobbyUpdated: (event) => {
                ...
                if (!debouncer) {
                    debouncer = new UpdateDebouncer((event) =>
                        io.emit(`lobby-updated`, event),
                    );
    ...
    private async handleConnection(socket: ClientSocket) {
        ...
        if (this.connections.has(connectionId)) {
            ...
            const transferredParticipations =
                await this.sessionManager.connectionTransfer(
                    oldConnection.id,
                    socket.id,
                );
        ...
        this.connections.set(
            `${clientInfo.deviceId}:${clientInfo.ephemeralClientId}`,
            socket,
        );
        ...
        {
            const reclaimedSessions =
                await this.sessionManager.connectionReclaimFromDeviceId(
                    clientInfo.deviceId ?? ``,
                    socket.id,
                );
        ...
        socket.emit(
            `shutdown-updated`,
            this.serverShutdownService.getShutdownState(),
        );
        ...
        this.bindSocketHandler(
            ...
            async (request) => {
                await participationMutex.runExclusive(async () => {
                    ...
                    const watchedSessions = this.getOrCreateWatchedSessions(
                        socket.id,
                    );
        ...
        this.bindSocketHandler(
            ...
            async (request) => {
                await participationMutex.runExclusive(async () => {
                    const watchedSessions = this.socketWatchedSessions.get(
                        socket.id,
                    );
            ...
            async (request) => {
                await participationMutex.runExclusive(async () => {
                    const [existingParticipation] =
                        this.sessionManager.getParticipationsBySocketId(
                            socket.id,
                        );
                    if (
                        existingParticipation?.session.id === request.sessionId
                    ) {
                    ...
                    if (existingParticipation) {
                        throw new SessionError(
                            `Socket already bound to a session`,
                        );
                    }

                    const session = this.sessionManager.requireSession(
                        request.sessionId,
                    );
                    const user =
                        authenticatedUser ??
                    ...
                    const { participant, role } =
                        await this.sessionManager.joinSession(session, {
                            ...
                            allowSelfJoinCasualGames:
                                preferences.allowSelfJoinCasualGames,
                        });
        ...
        this.bindSocketHandler(
            ...
            async ({ sessionId }) => {
                await participationMutex.runExclusive(async () => {
                    const participation =
                        socket.data.participations.get(sessionId);
                    if (!participation) {
                        return;
                    }
                    ...
                    if (
                        this.socketWatchedSessions
                            .get(socket.id)
                            ?.has(participation.sessionId)
                    ) {
        ...
        this.bindSocketHandler(
            ...
            async ({ sessionId }) => {
                await participationMutex.runExclusive(async () => {
                    ...
                    await this.sessionManager.surrenderSession(
                        session,
                        participantId,
                    );
                });
        ...
        this.bindSocketHandler(
            ...
            async ({ sessionId }) => {
                await participationMutex.runExclusive(async () => {
                    ...
                    await this.sessionManager.requestDraw(
                        session,
                        participantId,
                    );
                });
        ...
        this.bindSocketHandler(
            ...
            async ({ sessionId }) => {
                await participationMutex.runExclusive(async () => {
                    ...
                    await this.sessionManager.acceptDraw(
                        session,
                        participantId,
                    );
                });
        ...
        this.bindSocketHandler(
            ...
            async ({ sessionId }) => {
                await participationMutex.runExclusive(async () => {
                    ...
                    await this.sessionManager.declineDraw(
                        session,
                        participantId,
                    );
                });
                ...
                await participationMutex.runExclusive(async () => {
...

                    {
                        const session =
                            this.sessionManager.requireSession(sessionId);
                        const rematch =
                            await this.sessionManager.requestRematch(
                                session,
                                participantId,
                            );
        ...
        this.bindSocketHandler(
            ...
            async ({ sessionId }) => {
                await participationMutex.runExclusive(async () => {
                    ...
                    await this.sessionManager.cancelRematch(
                        session,
                        participantId,
                    );
                });
        ...
        this.bindSocketHandler(
            ...
            async (request) => {
                await participationMutex.runExclusive(async () => {
                    ...
                    await this.sessionManager.placeCell(
                        session,
                        participantId,
                        request.cell,
                    );
        ...
        this.bindSocketHandler(
            ...
            async (request) => {
                await participationMutex.runExclusive(async () => {
                    ...
                    this.sessionManager.sendChatMessage(
                        session,
                        participantId,
                        request.message,
                    );
    ...
    private putClientInGameState(
        socket: ClientSocket,
        participation: ClientGameParticipation,
    ) {
        socket.data.participations.set(participation.session.id, {
    ...
    emitTournamentNotification(
        profileId: string,
        event: TournamentNotificationEvent,
    ) {
        this.io
            ?.to(getProfileRoom(profileId))
            .emit(`tournament-notification`, event);
    }
    ...
    private bindSocketHandler<
        T extends keyof ClientToServerEvents,
        E = Parameters<ClientToServerEvents[T]>[0],
    >(
        socket: ClientSocket,
        eventType: T,
        ...
        socket.on(eventType as any, (rawEvent: unknown) => {
            let event: E;
            try {
                event = eventSchema.parse(rawEvent);
            } catch (error: unknown) {
                logSocketActionFailure(
                    this.logger,
                    `${eventType}:validate`,
                    socket,
                    error,
            ...
            callback(event).catch((error) => {
                logSocketActionFailure(this.logger, eventType, socket, error, {
                    event,
                });
                socket.emit(`error`, getSocketErrorMessage(error));
            });
