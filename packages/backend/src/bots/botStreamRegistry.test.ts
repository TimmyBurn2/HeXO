...
class FakeConnection implements BotStreamConnection {
    ...
    /** The socket dying without a close first: the error listener, then the close. */
    fail(error: Error): void {
        this.errorListener?.(error);
        this.destroy();
    }

    on(event: `close` | `error`, listener: (() => void) | ((error: Error) => void)): unknown {
        if (event === `close`) {
...
botTest(`a second stream over a live one is a swap, and the old close must not orphan the games`, async ({ sessionManager, session, registry, connection }) => {
    registry.open(BOT_PROFILE, connection, false);
    await settle();
    ...
    /* The swap already owns the games; the first stream's late close event is the
     * exact branch that must stay identity-guarded. */
    connection.destroy();
    await settle();
    assert.equal(registry.isOnline(BOT_PROFILE_ID), true);
    assert.equal(session.players.find((player) => player.id === BOT_SEAT)?.connection.status, `connected`);
    assert.deepEqual(second.events().map((event) => event.type), [`gameStart`]);

    await sessionManager.placeCell(session, HUMAN_SEAT, { x: 0, y: 0 });
    await settle();

    const request = second.events().filter((event) => event.type === `moveRequest`).at(-1);
    assert.equal(request?.type === `moveRequest` ? request.request.request_id : null, 1);
});

botTest(`a stream that errors rather than closing cleanly drops the bot, not the process`, async ({ session, registry, connection }) => {
    registry.open(BOT_PROFILE, connection, false);
    await settle();
    assert.equal(registry.isOnline(BOT_PROFILE_ID), true);

    connection.fail(new Error(`EPIPE`));
    await settle();
