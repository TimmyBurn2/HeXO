...
test(`with a real pool, a hung worker resigns its game and the next game is still served`, async () => {
    const sessionManager = createTestSessionManager();
    const pool = new EngineWorkerPool(pino({ level: `silent` }), { size: 1, graceMs: 100, entry: new URL(`./engineWorker.fixture.ts`, import.meta.url) });
    ...
    try {
        ...
        assert.equal(hanging.state, `finished`);
        ...
        assert.equal(healthy.gameState.cells.length, 3, `a fresh worker served the next think`);
